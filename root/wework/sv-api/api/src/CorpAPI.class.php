...
class CorpAPI extends API
{
    ...
    {
        $tkarr = \imcat\extCache::tkGet($this->tkKey); // 取缓存
     ...
     * @return : User array
     */
    public function UserSimpleList($department_id, $fetchChild, $getarr=0)
    {
        Utils::checkIsUInt($department_id, "department_id"); 
     ...
     * @return : Department array
     */
    public function DepartmentList($departmentId=null, $getarr=1)
    { 
    ...
    public function GetUserById($id=""){
        if($id){
            self::_HttpCall('/cgi-bin/user/get', 'GET', array('access_token'=>'ACCESS_TOKEN','userid'=>$id));
            return $this->rspJson;
        }else{
            return ['errcode'=>'errUserID', 'errmsg'=>'Error UserID @ GetUserById()'];
        }
    }
    ...
    static private function _HttpPostXml($url, $args)
    {
        ...
        return Utils::Xml2Array($rspRawStr); // $this->rspRawStr
    } 
