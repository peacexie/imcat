...
class WXBizMsgCrypt
{
	 ...
	 */
	public function __construct($token, $encodingAesKey, $receiveId, $enc_type='xml')
	{
		$this->m_sToken = $token;
	...
	*/
	 ...
	 *    <li>生成安全签名</li>
	 *    <li>将消息密文和安全签名打包成xml/json格式</li>
	 * </ol>
	 *
	 * @param $replyMsg string 公众平台待回复用户的消息，xml/json格式的字符串
	 * @param $timeStamp string 时间戳，可以自己生成，也可以用URL参数的timestamp
	 * @param $nonce string 随机串，可以自己生成，也可以用URL参数的nonce
	 * @param &$encryptMsg string 加密后的可以直接回复用户的密文，包括msg_signature, timestamp, nonce, encrypt的xml/json格式的字符串,
	 *                      当return返回0时有效
	 *
	...
	public function EncryptMsg($sReplyMsg, $sTimeStamp, $sNonce, &$sEncryptMsg)
	{
		...
		//生成发送的xml/json
		$userParse = $this->enc_type=='xml' ? new XMLParse : new JsonParse;
		$sEncryptMsg = $userParse->generate($encrypt, $signature, $sTimeStamp, $sNonce);
		return ErrorCode::$OK;
	}
	...
	public function DecryptMsg($sMsgSignature, $sTimeStamp = null, $sNonce, $sPostData, &$sMsg)
	{
		...
		//提取密文
		$userParse = $this->enc_type=='xml' ? new XMLParse : new JsonParse;
		$array = $userParse->extract($sPostData);
		$ret = $array[0];
