...
if($view=='set'){
    ...
    foreach($_groups as $mod=>$gv){
      ...
      echo $cv->Url(lang('flow.sh_json'),1,"?$mkv&view=json&mod=$mod","blank");
      echo $cv->Url(lang('flow.sh_tpl'),1,"?$mkv&view=tpl&mod=$mod","blank");
    ...
    if(!empty($bsend)){    
        ...
        $apiurl = PATH_BASE."?ajax-exdb&mod=$mod&act=".($view=='json' ? 'pull' : 'show')."";
