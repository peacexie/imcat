...
if($view=='list'){
    ...
    include dirname(__DIR__).'/binc/exd_inc1.php';
    ...
    foreach($list as $r){
      ...
      echo $cv->Url(lang('flow.title_cfg'),1,"?$mkv&mod=$r[mod]&view=fields&job=$r[kid]&recbk=ref","");
      ...
      echo $cv->Url(lang('flow.cw_crawl'),1,PATH_BASE."?ajax-exdb&act=crawl&mod=$r[mod]&job=$kid&".exdBase::getJSign(),'blank');
      echo $cv->Url(lang('flow.title_copy'),1,"?binc-exd_copy&mod=exd_oimp&kid=$r[kid]&type=tabid&title=$r[title]",lang('flow.oi_copy'),480,360); 
      echo "</tr>"; 
    }}
    echo "<tr>\n";
    ...
    if(!empty($bsend)){
        ...
        basMsg::show($msg,"Redir","?$mkv&view=$view&job=$job&mod=$mod&flag=v1"); 
    }
    ...
    $url = PATH_BASE."?ajax-exdb&act=crawl&mod=$fm[mod]&job=$job&debug=links&".exdBase::getJSign();
    ...
    if(!empty($bsend)){
        $vbak = $view;
        $view = 'del_b3';
        require dirname(__DIR__).'/binc/exd_inc1.php';
        $view = $vbak;
    } 
    ...
    if($rs=$dop->getRecs()){ 
        foreach($rs as $r){ 
          ...
          echo $cv->Url(lang('flow.op_upd'),1,PATH_BASE."?ajax-exdb&act=crawl&mod=$jcfg[mod]&job=$r[kid]&sysid=$r[sysid]&".exdBase::getJSign(),'blank');
          echo "</tr>";
        }
