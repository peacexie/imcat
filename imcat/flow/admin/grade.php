...
if($view=='glist'){
    ...
    if(!empty($bsend)){
        ...
        if(!empty($kid)){
            ...
            $vstr = "url='".PATH_BASE."?ajax-cajax&act=keyExists&mod=&tab=$tabid' tip='".lang('admin.fad_tip41245')."' ";
    ...
    if(!empty($bsend)){
        ...
        $lnkbak = "<a href='?$mkv&mod=$mod'>&lt;&lt;".lang('admin.grd_back')."[$gname]".lang('admin.grd_glist')."</a>";
        $lpart1 = " | "; $lpart2 = " | ";
        $pcfg1 = array('pmod'=>lang('admin.grd_mod'),'padd'=>lang('flow.dops_add'),'pdel'=>lang('flow.dops_del'),'pcheck'=>lang('flow.dops_checked'));
        foreach($pcfg1 as $k=>$v) { $lpart1 .= "<a href='?$mkv&mod=$mod&view=set&kid=$kid&parts=$k' ".(($parts==$k) ? 'class="cur"' : '').">$v</a> | "; }
        $pcfg2 = basLang::ucfg('cfglibs.grset_types');  
        foreach($pcfg2 as $k=>$v) { $lpart2 .= "<a href='?$mkv&mod=$mod&view=set&kid=$kid&parts=$k' ".(($parts==$k) ? 'class="cur"' : '').">$v</a> | "; }
        $sinit = "";
        if(in_array($parts,array('mkva','mkvu'))){
            $sinit = "<a href='?$mkv&mod=$mod&view=init&kid=$kid&parts=$parts' target='_blank' onclick='return winOpen(this,\"Init\");'>Init-Mkv:From-Menu</a><br>";
        }
