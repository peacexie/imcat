...
$gbar = ''; $ggap = ''; $cfg = array();
foreach($list as $r){
    $gbar .= "$ggap<a href='?$mkv&view=set&parts=$r[kid]' ".($parts==$r['kid'] ? 'class="cur"' : '').">$r[title]</a>";
    $ggap = ' | ';
    if($parts==$r['kid']){ 
        $cfg = $r;
...
if($view=='upd'){
    ...
    if(!empty($bsend)){
        ...
        if(!empty($kid)){
            ...
            $vstr = "tip='".lang('admin.fad_tip21245')."'"; //url='".PATH_BASE."?ajax-cajax&act=keyExists&mod=$mod&tab=$tabid' 
    ...
    $lnkbak = "<a href='?$mkv&view=list'>&lt;&lt;".lang('admin.rel_relist')."</a>";
