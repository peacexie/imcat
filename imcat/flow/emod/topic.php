...
if($act=='iform'){
    ...
    if(!empty($bsend)&&$fs_do=='dnow'){
        $msg = $dop->opDelnow();
        basMsg::show($msg,'Redir',"?$mkv&mod=topic&did=$did&view=formd&flag=v1");
    }
    ...
    if(!empty($bsend)){
      ...
      basMsg::show('设置成功!','Redir');  
        ...
        glbHtml::fmt_head('fmlist',"$aurl[1]",'tbdata');
        $inp = "<a class='right' href='?dops-a&mod=topic&view=tpls' onclick='return winOpen(this,\"模板浏览\",300,450);'>模板</a>";
    ...
    foreach ($fcfg as $k2=>$title) {
        ...
        $lnkurl = "?$mkv&mod=$mod&did=$did&dno=$dno&act=iform&view=$view&part=$k2&recbk=ref";
    ...
    if($rs=$dop->getRecs()){ 
        foreach($rs as $r){ 
            ...
            $lnkurl = "?$mkv&mod=$mod&did=$did&dno=$kid&act=iform&view=$view&part=$part&recbk=ref";
