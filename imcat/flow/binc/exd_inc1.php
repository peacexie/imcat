...
if($view=='set_a2'){
    ...
    if(empty($ocfgs)){
        ...
        <a href='?admin-ediy&part=edit&dkey=cfgs&dsub=&efile=excfg/ex_outdb.php' onclick=\"return winOpen(this,'".lang('flow.ei_editcfg')."',780,560);\">".lang('flow.title_edit')."</a></td></tr>\n";
        echo "\n<tr><td class='tc'>['psyn']['server']</td>\n<td>{$ocfgs['psyn']['server']}/index.php?ajax-exdb</td></tr>\n";
    ...
    if($tabid=='exd_crawl'){
        $url = PATH_BASE."?ajax-exdb&act=crawl&mod=$jcfg[mod]&job=$job&debug=field&field=$kid&url=".urlencode($jcfg['odmp'])."&".exdBase::getJSign();
    ...
    foreach($list as $r){
      ...
      if($r['dbtype']=='nodb' || in_array($kid,$fskip)){
          ...
          echo "<td class='tc'><a href='?$mkv&mod=$mod&view=fset&job=$job&kid=$r[kid]' onclick='return winOpen(this,\"".lang('flow.ei_fldcfg')."\")'>".lang('flow.title_cfg')."</a></td>\n";
      }
    ...
    foreach($dop->fext as $fk=>$fv){
      ...
      echo "<td class='tc'><a href='?$mkv&mod=$mod&view=fset&job=$job&kid=$fk' onclick='return winOpen(this,\"".lang('flow.title_cfg')."\")'>".lang('flow.title_cfg')."</a></td>\n";
