...
class dopUser extends dopBase{	
	...
	function sobar($msg='',$width=30){ 
		...
		$sbar .= "\n&nbsp; ".$this->so->Order(array('uname' => '账号(降)','uname-a' => '账号(升)',));
	...
	function fmAccount(){ 
		...
		glbHtml::fmae_row('登陆名',$this->fmSetUname().' &nbsp; 密码'.$this->fmSetPW());
		//glbHtml::fmae_row('问题',$this->fmSetQA('getpwq').' &nbsp; 答案'.$this->fmSetQA('getpwa'));
		glbHtml::fmae_row('等级',$this->fmType('grade',180).' &nbsp; 显示'.$this->fmShow());
	...
	function fmSetPW(){
		...
		$item = "<input id='fm[upass]' name='fm[upass]' type='text' value='' maxlength=24 class='txt w180' reg='{$nultip}key:6-24' tip='6-24字符' />$nulmsg";
		return $item;
	}
	...
	function fmSetUname(){
		...
		$prop = " id='fm[uname]' name='fm[uname]' type='text' maxlength=24 reg='$len' tip='字母开头,允许字母数字下划线<br>允许3-15字符,建议4-6字符' ";
		if(empty($val)){
