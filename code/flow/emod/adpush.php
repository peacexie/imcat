...
if($view=='list'){
    ...
    if($rs=$dop->getRecs()){ 
        foreach($rs as $r){ 
          ...
          echo $cv->Url('Page',1,PATH_PROJ.str_replace(array("?"),array("?mkv="),$r['url'])."&adpush=load","blank");
    ...
    if(!empty($bsend)){
        ...
        if(!empty($omkv) && !empty($otpl)){
            $url = "/plus/ajax/cron.php?static=$omkv&tpldir=$otpl&act=mkv";
            echo "\n<iframe src='".PATH_ROOT."$url"."' width='100%'></iframe>"; // static
        }
