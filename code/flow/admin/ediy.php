...
$lfile = "<a href='?mkv=admin-files'>SysFiles</a> # ";
...
if(in_array($part,array('edit','restore','down'))){
    ...
    if($part=='restore'){
        ...
        glbHtml::fmt_head('fmlist',"?mkv=$mkv&part=edit&dkey=$dkey&dsub=$dsub&efile=$efile",'tblist');
    ...
    glbHtml::fmt_head('fmlist',"?mkv=$mkv&part=$part&dkey=$dkey",'tblist');
    ...
    if($dkey=='runs'){
        ...
        foreach($lists['dir'] as $sdir=>$etime){
            ...
            $lnkds .= (empty($lnkds)?'':' # ')."<a href='?mkv=$mkv&part=$part&dkey=$dkey&dsub=$sdir'>$ititle</a>";
    ...
    foreach($listu as $ifile=>$fv){ 
      ...
      if($dkey=='dtmp'){
          echo $cv->Url('Down','1',"?mkv=$mkv&part=down&dkey=$dkey&dsub=$dsub&efile=$bkfile");
      }elseif(file_exists(DIR_PROJ.$edir."/$bkfile.maobak")){
          echo $cv->Url(lang('admin.ediy_rebak'),'1',"?mkv=$mkv&part=restore&dkey=$dkey&dsub=$dsub&efile=$bkfile");
      }else{
          echo "<td class='tc cCCC'>".lang('admin.ediy_rebak')."</td>\n";
      }
      echo $cv->Url(lang('flow.dops_edit'),'1',"?mkv=$mkv&part=edit&dkey=$dkey&dsub=$dsub&efile=$bkfile",lang('admin.ediy_edit').":{$bkfile}");
      echo "</tr>"; 
    }
