...
$pid = empty($pid) ? '0' : $pid;
if(!($gname = @$_groups[$mod]['title'])) glbHtml::end('参数错误:mod1@catalog.php');

$cfg = glbConfig::read($mod); 
$tabid = "base_catalog";
$gbar = admAFunc::grpNav($cfg['pid'],$mod); 
if(!in_array($cfg['pid'],array('docs','advs'))) glbHtml::end('参数错误:mod2@catalog.php');
...
if($view=='glist'){
	...
	glbHtml::tab_bar("[$lnkbak]栏目 :: $gname<span class='span ph5'>|</span>$lnkadd<br>$lnklay",$gbar,35);
	...
	if(in_array($mod,$_ex_paras['catid'])) echo "<th>参数</th>";
	...
	foreach($list as $r){
	  ...
	  if(in_array($mod,$_ex_paras['catid'])) echo "<td class='tc'>".("<a href='?file=admin/fields&mod=$mod&catid=$r[kid]'>设置</a>")."</td>\n";  
	...
	echo "<td class='tr' colspan='$cspan'><span class='cF00 left'>$msg</span>批量操作: <select name='fs_do'>".basElm::setOption("upd|更新\ndel|删除\nshow|启用\nstop|禁用\nframe|结构\nnofrm|非结构")."</select> <input name='bsend' class='btn' type='submit' value='执行操作' /> &nbsp; </td>\n";
	echo "</tr>";
	...
	if(!empty($bsend)){
		...
		if(!empty($kid)){
			...
			$vstr = "url='".PATH_ROOT."/plus/ajax/cajax.php?act=keyExists&mod=$mod&tab=$tabid' tip='字母开头,允许字母数字下划线<br>允许4-12字符,建议4-5字符'";
