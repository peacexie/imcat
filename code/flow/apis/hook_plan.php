...
if($view=='list'){
    ...
    glbHtml::tab_bar("".lang('flow.cr_hookord')."<span class='span ph5'>|</span>$lnkadd","$links",50);
    
    $list = $db->table($tabid)->where("hkflag='1'")->order('top')->select(); 
    glbHtml::fmt_head('fmlist',"$aurl[1]",'tblist');
    echo "<th>".lang('flow.title_select')."</th><th>".lang('flow.cr_kfine')."</th><th>".lang('flow.title_name')."</th><th>".lang('flow.cr_cycle')."</th><th>".lang('flow.cr_runprev')."</th><th>".lang('flow.cr_runnext')."</th><th>mkv/tpl</th><th>".lang('flow.title_top')."</th><th>".lang('flow.title_enable')."</th><th>".lang('flow.title_edit')."</th>\n";
    if($list){
    foreach($list as $r){
      $kid = $r['kid']; 
      echo "<tr>\n".$cv->Select($kid);
      ...
      echo "<td class='tc'>$r[hkmkv]/$r[hktpl]</td>\n";
    ...
    if(!empty($bsend)){
        ...
        $def = array(
            'kid'=>'','title'=>'','top'=>'888','enable'=>'1','note'=>'','cfgs'=>'','hkmkv'=>'0','hktpl'=>'0',
        ...
        if(!empty($kid)){
            ...
            $vstr = "tip='".lang('flow.hk_tip1')."'"; //url='".PATH_ROOT."/plus/ajax/cajax.php?act=modExists' 
